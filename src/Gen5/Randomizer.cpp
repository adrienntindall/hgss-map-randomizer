...
#define WARP_DATA "./files/WarpsBW2.csv"
#define WARP_DICT  "./files/WarpDictionaryBW2.csv"
...
void GetBlocks(string blockPath) {
    ...
    for(const auto & file : recursive_directory_iterator(blockPath)) {
        ...
        if(comp == "blk") { //BLocKs
            ...
            if(path.find("AspertiaCity") != string::npos) {
...
void GetWarpDict() {
    ...
    getline(warpDict, line); //burns the first line lol
...
void SetWarps() {
    ...
    for(const auto & file : directory_iterator(FILE_PATH)) {
        ...
        //Skipping to the warp amounts
        pos = 8 + readChar(binData, 0x4) * 0x14 + readChar(binData, 0x5) * 0x24;
        
        //Reading in number of warps
        unsigned int warpAmount = readChar(binData, 0x6);
        
        for(int w = 0; w < warpAmount; w++) {
            //First short is the map header in BW2
            short mapHeaderID = readShort(binData, pos);
            //Next short is the map anchor, and luckily this is all the info we need!
            ...
            pos += 0x14; //this skips us ahead past the height and to the beginning of the next warp =)
        }
